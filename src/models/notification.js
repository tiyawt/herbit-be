...
export const NOTIF_TYPES = ["daily_task", "ecoenzym_project", "game_sorting", "voucher"];
...
notificationSchema.index(
  ...
  { unique: true, partialFilterExpression: { reference_id: { $type: "objectId" } } }
);

// Khusus daily_task: cegah > 1 notifikasi per hari per user
notificationSchema.index(
  { user_id: 1, type: 1, day_bucket: 1 },
  { unique: true, partialFilterExpression: { day_bucket: { $type: "string" }, type: "daily_task" } }
);
